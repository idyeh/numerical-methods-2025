...
## 重心公式推导
从拉格朗日形式起步。把 $\ell_i(x)$ 重写为

$$
\ell_i(x)=\frac{\prod_{j\ne i}(x-x_j)}{\prod_{j\ne i}(x_i-x_j)}
=\frac{\prod_{j=0}^{n}(x-x_j)}{x-x_i}\cdot
\underbrace{\frac{1}{\prod_{j\ne i}(x_i-x_j)}}_{w_i}
$$
...
- 若 $x$ 与某个 $x_k$**恰等**，直接返回 $y_k$（避免 0/0）。批量求值时可先做相等掩码。
...
fig, axes = plt.subplots(len(orders), 2, figsize=(10, 10), sharex=True, sharey=True)
...
- 目的：在同一节点与同一求值网格上，比较 **NumPy** 与 **JAX（CPU, 64-bit, `jit`+`vmap`）** 的 **重心插值**效果与性能。同时对比等距节点与第二类切比雪夫节点。
- 要点：
  - JAX 版本采用**通用权重** $w_i = 1/\prod_{j\ne i}(x_i-x_j)$ 的 O($n^2$) 实现；  
  - 求值阶段使用 `vmap` 批量化 + `jit` 编译，避免 Python 循环；  
  - 64-bit 开启：`jax.config.update("jax_enable_x64", True)`；  
